...
| `interpreter_factory` | `None` | Replaces the default `dspy.PythonInterpreter` factory for code-executing modules ([`dspy.RLM`](../modules/RLM.md), [`dspy.Flex`](../modules/Flex.md), `dspy.ProgramOfThought`, `dspy.CodeAct`). A non-default factory passed to a module's constructor takes priority. When `None`, modules use [`dspy.PythonInterpreter`](../../diving-deeper/rlm.md). |
