...
Both rely on `MontyInterpreter`, which runs LM-generated code in a sandbox via the Monty runtime. Monty isolation is the reason: the LM’s code runs in a process with no filesystem or network access by default, so executing untrusted output is bounded. Each invocation gets a fresh interpreter, while retries and iterations within that invocation share its state.
