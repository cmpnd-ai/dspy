...
| `interpreter_factory` | `None` (uses `PythonInterpreter`) | Replaces the default `PythonInterpreter` factory for code-executing modules. A non-default factory passed to a module's constructor takes priority. |
