...
class Adapter:
    ...
    def format(
        ...
        return [message_to_openai_chat(message) for message in self._format_request_messages(state, demos)]

    @with_callbacks
    def _format_request_with_callbacks(self, state: _AdapterRequestState, demos: list[dict[str, Any]]) -> list[LMMessage]:
