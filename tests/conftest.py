...
@pytest.fixture
def monty_interpreter():
    """Provide a fresh Monty interpreter for behavioral integration tests."""
...
@pytest.fixture
def configure_monty_interpreter(monty_interpreter):
    """Configure the per-test Monty interpreter with tools and output fields."""

    def configure(tools: dict[str, Any] | None = None, output_fields: list[dict[str, Any]] | None = None):
        if tools:
            monty_interpreter.tools.update(tools)
        if output_fields is not None:
            monty_interpreter.output_fields = output_fields
        return monty_interpreter
