...
# A single LM response that satisfies both the `question -> answer` predictor
# (which extracts `answer`) and the `OfferFeedback` predictor (which extracts
# `discussion` and `advice`). Using one combined response makes the LM answer
# order irrelevant, so consumption stays correct across multiple forward() calls
# (where each call consumes an odd number of responses: predictor + advice pairs
# plus a final predictor without advice).
ANSWER_AND_ADVICE = {"answer": "Brussels", "discussion": "no blame", "advice": {"predictor": "N/A"}}


def test_refine_fail_count_tolerates_single_late_failure():
    ...
    dspy.configure(lm=DummyLM([ANSWER_AND_ADVICE] * 8))
    ...
    refine = Refine(module=predict, N=5, reward_fn=lambda _, __: 1.0, threshold=2.0, fail_count=2)
...
def test_refine_fail_count_still_raises_when_exceeded():
    ...
    refine = Refine(module=predict, N=6, reward_fn=lambda _, __: 1.0, threshold=2.0, fail_count=1)
...
def test_refine_fail_count_does_not_leak_across_calls():
    ...
    dspy.configure(lm=DummyLM([ANSWER_AND_ADVICE] * 20))
    ...
    r1 = refine(question="Q?")
    assert r1.answer == "Brussels"
    assert refine.fail_count == 2

    state["rollout_in_call"] = 0
    r2 = refine(question="Q?")
